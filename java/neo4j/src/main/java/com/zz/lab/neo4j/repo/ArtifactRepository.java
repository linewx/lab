...
public interface ArtifactRepository extends Neo4jRepository<Artifact, Long> {
    ...
    Artifact findOneByArtifactIdAndGroupId(String artifactId, String groupId, @Depth int depth);
    ...
    @Query("MATCH (a:Artifact) where size((a)-[:deps]->()) = {outDegree} and size((a)<-[:deps]-()) = {inDegree} RETURN a")
